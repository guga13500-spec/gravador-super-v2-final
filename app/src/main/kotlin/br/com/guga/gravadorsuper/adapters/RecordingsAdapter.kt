...
import br.com.guga.gravadorsuper.dialogs.DeleteConfirmationDialog
...
import br.com.guga.gravadorsuper.extensions.trashRecordings
...
import br.com.guga.gravadorsuper.dialogs.TrimRecordingDialog
import br.com.guga.gravadorsuper.helpers.AudioHelper
import java.io.File
...
class RecordingsAdapter(
    ...
    override fun prepareActionMode(menu: Menu) {
        menu.apply {
            ...
            findItem(R.id.cab_trim).isVisible = isOneItemSelected()
    ...
    private fun trimRecording() {
        ...
        var outputPath = "$parent/${name}_trimmed.$extension"
        var i = 1
        while (File(outputPath).exists()) {
            outputPath = "$parent/${name}_trimmed_$i.$extension"
            i++
        }

        TrimRecordingDialog(activity, recording) { start, end ->
            ensureBackgroundThread {
                AudioHelper.trimAudio(activity, path, outputPath, start.toLong() * 1000, end.toLong() * 1000)
